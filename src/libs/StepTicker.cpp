...
void StepTicker::step_tick (void)
{
    ...
    // Service camera/strobe pulse widths with integer-only ISR work.
    ...
    // Fly-by trigger path: integer flag/tick comparison only. The callback is ISR-safe.
    ...
    if(!still_moving) {
        ...
        // Blocks are recycled by the conveyor; do not carry stale trigger metadata.
...
bool StepTicker::start_next_block()
{
    ...
    for (uint8_t m = 0; m < num_motors; m++) {
        ...
        // NOTE this would be at least 5us before first step pulse.
